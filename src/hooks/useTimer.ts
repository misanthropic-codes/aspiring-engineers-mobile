/**
 * useTimer Hook - Test Portal Mobile
 * 
 * Timer hook for test duration tracking.
 * Ported from test-portal-client/src/hooks/useTimer.ts
 */
        ...
        setRemainingSeconds((prev) => {
          ...
          if (onWarning) {
            warningThresholds.forEach((threshold) => {
              if (
                newValue === threshold &&
                !warningsGivenRef.current.has(threshold)
              ) {
